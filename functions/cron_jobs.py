...
from functions.skills_runtime import SkillRuntime, set_skill_enabled
...
CRON_DIR = APP_DIR / "personal_data" / "cron_data"
...
def queue_outbound_message(
    ...
    *user_id* identifies which WhatsApp connection should deliver the message.
    When ``None``, the bridge will try all connected users (legacy fallback).
    """
    msg = {
    ...
    with _lock:
        ...
        _save_outbound()
    logger.info("Queued outbound message %s for %s (user_id=%s)", msg["msg_id"], target_jid, user_id)
    return msg
...
def _scheduler_loop() -> None:
    ...
    while _scheduler_running:
        ...
        # ── Heartbeat check ──────────────────────────────────────────
        try:
            from functions.heartbeat import run_heartbeat
            run_heartbeat()
        except Exception as e:
...
@cron_bp.route("/settings")
@login_required
def settings_page():
    """Settings root route."""
    return redirect(url_for("cron_jobs.settings_cron_jobs_page"))


@cron_bp.route("/settings/cron-jobs")
@login_required
...
@cron_bp.route("/settings/heartbeat")
@login_required
def settings_heartbeat_page():
    """Render heartbeat settings page."""
    return render_template(
        "settings_heartbeat.html",
        username=_username(),
        settings_section="heartbeat",
    )


@cron_bp.route("/api/heartbeat/config")
def api_heartbeat_config():
    """Get heartbeat configuration."""
    if not _require_login():
        return jsonify(success=False, message="Login required"), 401
    from functions.heartbeat import load_config
    return jsonify(success=True, config=load_config())


@cron_bp.route("/api/heartbeat/config", methods=["PUT"])
def api_update_heartbeat_config():
    """Update heartbeat configuration."""
    if not _require_login():
        return jsonify(success=False, message="Login required"), 401

    data = request.get_json(force=True)
    from functions.heartbeat import load_config, save_config
    ...
    if "interval_minutes" in data:
        ...
        if not (5 <= val <= 360):
            return jsonify(success=False, message="interval_minutes must be 5-360"), 400
        config["interval_minutes"] = val

    if "active_hours_start" in data:
        if not re.match(r"^\d{2}:\d{2}$", data["active_hours_start"]):
            return jsonify(success=False, message="Invalid time format"), 400
        config["active_hours_start"] = data["active_hours_start"]

    if "active_hours_end" in data:
        if not re.match(r"^\d{2}:\d{2}$", data["active_hours_end"]):
            return jsonify(success=False, message="Invalid time format"), 400
        config["active_hours_end"] = data["active_hours_end"]

    if "max_messages_per_day" in data:
        val = int(data["max_messages_per_day"])
        if not (1 <= val <= 50):
            return jsonify(success=False, message="max_messages_per_day must be 1-50"), 400
        config["max_messages_per_day"] = val

    if "delivery_method" in data:
        if data["delivery_method"] not in ("whatsapp", "web"):
            return jsonify(success=False, message="delivery_method must be 'whatsapp' or 'web'"), 400
        config["delivery_method"] = data["delivery_method"]

    for key in ("enabled", "target_jid", "target_session_id", "username"):
        if key in data:
            config[key] = data[key]
...
@cron_bp.route("/api/heartbeat/status")
def api_heartbeat_status():
    """Get heartbeat status (last run, messages today, etc.)."""
    if not _require_login():
        return jsonify(success=False, message="Login required"), 401
    from functions.heartbeat import get_heartbeat_status
    return jsonify(success=True, status=get_heartbeat_status())


@cron_bp.route("/settings/devices")
@login_required
def settings_devices_page():
    """Render device connection settings page."""
    return render_template(
...
@cron_bp.route("/settings/skills")
@login_required
def settings_skills_page():
    """Render settings skills page."""
    return render_template(
...
@login_required
def cron_jobs_page():
    """Backward-compatible route: redirect cron jobs page to settings."""
    return redirect(url_for("cron_jobs.settings_cron_jobs_page"))
...
@cron_bp.route("/api/settings/skills")
def api_list_skills():
    """List available skills with current enabled state."""
    if not _require_login():
        return jsonify(success=False, message="Login required"), 401

    runtime = SkillRuntime()
    return jsonify(success=True, skills=runtime.list_skills())


@cron_bp.route("/api/settings/skills/<skill_id>/toggle", methods=["POST"])
def api_toggle_skill(skill_id: str):
    ...
    runtime = SkillRuntime()
    all_skills = {item["id"]: item for item in runtime.list_skills()}
    ...
    if "enabled" in data:
        ...
        enabled = not bool(all_skills[skill_id]["enabled"])
...
def api_whatsapp_contacts():
    ...
    Requires query param ?user_id=<int> to specify which user's contacts to list.
    Falls back to the logged-in user's id when not provided.
    """
    if not _require_login():
        return jsonify(success=False, message="Login required"), 401

    from whatsapp.flask_whatsapp import load_session_map_for_user
    ...
    session_map = load_session_map_for_user(user_id)

    contacts = []
    for jid, entry in session_map.items():
